...
export default function MiddleCategoryPage() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        setLoading(true);
        const url = `/api/products?main=${encodeURIComponent(main)}&middle=${encodeURIComponent(middle)}`;
    ...
    <div className="min-h-screen bg-white font-serif">
      ...
      <div className="pt-28 px-4 sm:px-6 md:px-8 lg:px-12">
        ...
        {loading ? (
          ...
          <p className="text-center py-20">No products found for {main} / {middle}</p>
        ) : (
